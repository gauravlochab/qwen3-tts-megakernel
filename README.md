...
| Perf: TTFC | ⚠️ ~80 ms warm median *(target <60 ms; **162 → 80 ms** via batched prefill, min 59 ms; remaining floor is codec + first frames)* | [Performance](#performance) |
...
| Streaming TTFC | **~80 ms median** (warm; min 59 ms — prefill 13 ms after batched bridge / codec 17 ms / code-pred 11 ms / stream overhead) | <60 ms | ⚠️ 2× (162 → 80, dips to 59) |
...
**Honest bottleneck analysis.** The megakernel makes the talker trunk ~5× cheaper, but the code-predictor was ~71% of the remaining budget — so we accelerated it. Profiling showed its 15 depth-steps/frame are **launch-latency-bound, not compute-bound** (the per-frame arithmetic is ~24 µs; the cost is dispatching many tiny kernels), so the win is *fewer kernels*, not a custom kernel: a **hand-written GQA forward + whole-frame Inductor fusion + a single CUDA-graph capture** ([§5a](bench/results.md)) takes the code-predictor from ~10.8 to ~3.0 ms/frame and **end-to-end RTF 0.77 → ~0.11**, clearing the **<0.15** target in pure bf16 (no quantization, no megakernel changes). Correctness held to a teacher-forced logit cosine of **0.9999** vs stock and a first-step greedy match of **100%** (later-step divergence is autoregressive amplification of fp near-ties, not a forward bug). For **TTFC**, the talker prefill was a per-token loop of 110 single-token kernel steps (~87 ms — the dominant term). Since the kernel stores post-qk-norm/post-RoPE K and raw V *identically* to HuggingFace's cache, we run the prefill as **one batched reference forward and bridge its per-layer K/V straight into the kernel's cache** (then the kernel does the single-token decode as before). Validated: the post-bridge decode hidden matches the full reference at **0.9999** — actually *better* than the per-token path, which accumulates the kernel's per-layer ~1e-4 across 28 layers. Prefill **87 → 13 ms**, warm TTFC **162 → ~80 ms median (min 59 ms)**. So **RTF<0.15 is met**; TTFC is now ~80 ms — not reliably under 60 ms, but its floor is now the codec (~17 ms, PyTorch per the task) + the first code-predictor frames (~11 ms) + streaming queue/scheduling overhead, each **measured and attributed** rather than hand-waved.
